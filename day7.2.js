...
// rewrite for aba / bab
function validSSL(ipa) {
  const abas = [];
  const babs = [];
  const chunks = ipa.split(/[\[\]]/);
  // odd chunks in brkts -- bab
  for (let i = 1; i < chunks.length; i += 2) {
    for (let j = 0; j < chunks[i].length - 2; j++) {
      let sub = chunks[i].substr(j, 3);
      if(isABA(sub)) {
        let foo = sub[1] + sub[0] + sub[1];
        babs.push(foo);
      }
    }
  }
  // even chunks outside brkts -- aba
...
// write fns for aba and bab
// maybe get rid of this function
// function hasABA(str) {
//   for (let i = 0; i < str.length - 2; i++) {
//     if(str.substr(i, 3) === str.substr(i, 3).split('').reverse().join('')) {
