// return obj mapping letters to freq
function letterCount(room) {
  const count = {};
  let roomArr = room.split('-');
  roomArr.pop();
  roomArr = roomArr.join('').split('');
  roomArr.forEach((char) => {
    // if char in obj
    // inc count[char]
    // else
    // count[char] = 1
...
// determines if room is real
function realRoom(room) {
  const count = letterCount(room);
  var keysSorted = Object.keys(count).sort((a, b) => {
    if (count[a] === count[b]) {
      if (a > b) {
        return 1;
      } else {
  ...
  var expectedChecksum = keysSorted.slice(0, 5).join('');
  // compare expected to listed
  // extract listed checksum from room str
  const listedChecksum = room.split('[')[1].substr(0, 5);
  // console.log('expected: ' + expectedChecksum);
  // console.log('listed: '   + listedChecksum);
  if (listedChecksum === expectedChecksum) {
    // console.log(true);
    return true;
  } else {
    // console.log(false);
...
function getSector(room) {
  const arr  = room.split('-');
  const last = arr.pop();
  return last.split('[')[0] * 1;
}

function rotate(char, count) {
  if (char === '-') {
    return ' ';
  }
  let newCharCode = char.charCodeAt(0) + count;
...
function decrypt(room) {
  ...
  for (let i = 0; i < roomName.length; i++) {
    decName += rotate(roomName[i], sector);
  }
  if (decName.includes('north')) {
    console.log(decName + sector);
  }
  // console.log(decName + ': ' + sector); 
}
...
const puzInput = `gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
